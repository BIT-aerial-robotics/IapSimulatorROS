...
void CommandsReceiver::receiveCommandsAndApply(AvaRobotCommand& _commands, const AvaRobotState& _states)
{
    ...
    _commands.thrust3 = thrustCallbackNo3_.get().data;
    ...
    _commands.moment3.x() = momentCallbackNo3_.get().x;
    _commands.moment3.y() = momentCallbackNo3_.get().y;
    _commands.moment3.z() = momentCallbackNo3_.get().z;

    Eigen::Vector3d force0, force1, force2, force3; // active force in their own frame
    ...
    Eigen::Matrix3d matrix1_origin = getRotationMatrix(originalEulerAngle1_); // original rotation from joint1 to base frame
    Eigen::Matrix3d matrix2_origin = getRotationMatrix(originalEulerAngle2_); // original rotation from joint2 to base frame
    Eigen::Matrix3d matrix3_origin = getRotationMatrix(originalEulerAngle3_); // original rotation from joint3 to base frame

    Eigen::Vector3d e3(0, 0, 1);
    force1 = -_commands.thrust1 * e3;
    force2 = -_commands.thrust2 * e3;
    force3 = -_commands.thrust3 * e3;
    force0 = matrix1_origin * _states.rotation1_0 * force1 +
             matrix2_origin * _states.rotation2_0 * force2 +
             matrix3_origin * _states.rotation3_0 * force3;
    moment0 = relativePosition1_.cross(matrix1_origin * _states.rotation1_0 * force1) +
              relativePosition2_.cross(matrix2_origin * _states.rotation2_0 * force2) +
              relativePosition3_.cross(matrix3_origin * _states.rotation3_0 * force3);
    moment_reaction = -(matrix1_origin * _states.rotation1_0 * _commands.moment1 +
                        matrix2_origin * _states.rotation2_0 * _commands.moment2 +
                        matrix3_origin * _states.rotation3_0 * _commands.moment3);
    ...
    Eigen::Vector3d moment3_0 = convertFromFluToFrd(_states.rotation3_0 * _commands.moment3);
    robot_->setGeneralizedForce({force0_w.x(),       force0_w.y(),       force0_w.z(),
                                 ...
                                 moment3_0.x(),      moment3_0.y(),      moment3_0.z()});
}
...
void CommandsReceiver::initializeSubscriber()
{
    ...
    thrustSubscriberNo3_ = nodeHandle_.subscribe<std_msgs::Float64>
                           (robot_->getName() + "/thrust_3", 1, &Thrust::callback, &thrustCallbackNo3_);
    ...
    momentSubscriberNo3_ = nodeHandle_.subscribe<geometry_msgs::Point>
                           (robot_->getName() + "/moment_3", 1, &Moment::callback, &momentCallbackNo3_);
}
